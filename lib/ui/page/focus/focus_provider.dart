...
class FocusProvider extends ChangeNotifier {
  ...
  Future<void> enterFinishState() async {
    ...
    await loadTotalMinute();
    await NotificationService.instance.showRunningNotification(title: '恭喜完成了专注', body: "",);
...
extension FocusProviderAction on FocusProvider {
  ...
  Future<void> clkCancel() async {
    ...
    await NotificationService.instance.cancelRunningNotification();
    await NotificationService.instance.cancelCompletedNotification();
    await clearSessionDataAndEnterState(FocusState.setting);
  }

  Future<void> clkBack() async{
    await clearSessionDataAndEnterState(FocusState.setting);
  }

  Future<void> clearSessionDataAndEnterState(FocusState state) async {
  ...
  Future<void> updateCurrentRecord(FocusRecord record) async {
    await FocusRecordRepository.instance.update(record);
    session = session.copyWith(
      currentTagId: record.tagId,
    );
    await saveSession();
    notifyListeners();
  }
